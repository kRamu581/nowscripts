...
export default function AdminDashboard() {
 ...
 <h2 className="text-lg font-semibold text-gray-900 mb-6">User Growth (Last 30 Days)</h2>
 ...
 <span className="text-sm font-medium text-gray-900 ">{health?.memoryUsage}%</span>
 </div>
 ...
 {liveUsers.length === 0 ? (
 <tr>
 <td colSpan={4} className="px-6 py-8 text-center text-gray-500">No users currently online.</td>
 </tr>
 ) : (
 liveUsers.slice(0, 5).map((u) => (
 <tr key={u.socketId} className="hover:bg-gray-50 :bg-gray-800/50 transition-colors">
 ...
 <tbody className="divide-y divide-gray-200 text-gray-700 ">
 {recentUsers.map((u) => (
 <tr key={u._id} className="hover:bg-gray-50 :bg-gray-800/50 transition-colors">
 <td className="px-6 py-4">
 <div className="flex items-center gap-3">
 <img src={u.avatar} alt="" className="w-8 h-8 rounded-full bg-gray-200" />
 ...
 <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${u.isActive ? 'bg-emerald-100 text-emerald-800 ' : 'bg-red-100 text-red-800 '}`}>
 ...
 <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 opacity-75 relative overflow-hidden group">
 <div className="absolute inset-0 bg-gray-50/50 backdrop-blur-[1px] z-10 flex items-center justify-center transition-opacity">
...
function HealthMetric({ label, value, status }: any) {
 ...
 <span className={`text-sm font-medium ${status === 'good' ? 'text-emerald-600 ' : 'text-amber-600 '}`}>
 {value}
 </span>
 </div>
 );
...
function ContentStatRow({ icon, label, value }: any) {
 return (
 <div className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 :bg-gray-800/50 transition-colors border border-transparent hover:border-gray-200 :border-gray-700">
 <div className="flex items-center gap-3">
