...
function PageLayout({ title, description, icon, lastUpdated, children }: { title: string, description: string, icon: React.ReactNode, lastUpdated?: string, children: React.ReactNode }) {
  React.useEffect(() => {
  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen pb-20 font-sans selection:bg-black selection:text-white">
      ...
      <div className="bg-white border-b border-gray-200/60 pt-16 pb-16 px-6 relative overflow-hidden">
        {/* Decorative elements */}
    ...
    >
        ...
        {[
          {
            ...
            desc: "Select a specialized roadmap tailored to your career goals—whether you want to become a System Administrator, Developer, or Architect.",
            icon: <Target className="w-8 h-8 text-[#FF338D]" />
          },
          {
            step: "02",
            title: "Learn & Practice",
            desc: "Dive into our comprehensive modules. Read expertly crafted notes, watch tutorials, and practice with our interactive flashcards.",
            icon: <BookOpen className="w-8 h-8 text-[#C557F8]" />
          },
          {
            step: "03",
            title: "Build Real Projects",
            desc: "Apply your knowledge by building end-to-end ServiceNow applications. Gain hands-on experience that you can showcase to employers.",
            icon: <Zap className="w-8 h-8 text-[#4353ED]" />
          },
          {
            step: "04",
            title: "Ace the Interview",
            desc: "Use our AI-powered interview prep dashboard to practice real-world questions, read community experiences, and walk into your interview with confidence.",
            icon: <MessageSquare className="w-8 h-8 text-black" />
          }
        ].map((item, i) => (
          <div key={i} className="flex flex-col md:flex-row gap-8 items-start bg-white p-8 md:p-10 rounded-3xl border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300">
      ...
      <div className="mt-20 bg-gradient-to-r from-[#FF338D] via-[#C557F8] to-[#4353ED] p-1 rounded-3xl">
        <div className="bg-white rounded-[22px] p-12 text-center">
          ...
          <button className="bg-black text-white px-10 py-4 rounded-full font-bold hover:scale-105 transition-transform shadow-xl text-lg">
