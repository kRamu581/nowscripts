...
function HomeContainer({ tag }: { tag: string }) {
 ...
 <div className="mb-8">
 <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-4 pl-3">Community</h3>
 ...
 : "text-[#475569] hover:bg-white hover:text-gray-900 "
 }`}
 >
 {category.icon}
 {category.label}
 ...
 <div>
 <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-4 pl-3">Popular Tags</h3>
 <div className="flex flex-wrap gap-2 pl-3">
 {POPULAR_TAGS.slice(0, 8).map(t => (
 <Link 
 key={t}
 to={`/tag/${t}`}
 className="text-xs font-medium bg-white border border-gray-200 text-gray-500 px-2.5 py-1.5 rounded-md hover:border-[#00C08B] hover:text-[#00C08B] transition-colors"
 >
 #{t}
 </Link>
 ))}
 ...
 <p className="text-gray-500 mt-1 text-sm font-medium">Join the ServiceNow learning ecosystem.</p>
 </div>
 ...
 className="w-64 pl-9 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#00C08B] transition-colors shadow-sm"
 />
 </div>
 {isAuthenticated ? (
 <Link 
 ...
 key={item.post._id}
 className="bg-white border border-gray-200 rounded-xl p-5 hover:border-[#00C08B] hover:shadow-md transition-all group relative overflow-hidden"
 >
 ...
 <h3 className="font-bold text-gray-900 line-clamp-2 leading-snug mb-3 group-hover:text-[#00C08B] transition-colors relative z-10">
 {item.post.title}
 </h3>
 ...
 : "bg-white border border-gray-200 text-[#475569]"
 }`}
 >
 {category.label}
 </button>
 ...
 <p className="text-gray-500 text-sm mb-8">But the community is alive! Check out what's happening right now.</p>
 
 {/* Rich Empty State */}
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
 {trendingDiscussions.length > 0 && (
 <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 ">
 <h4 className="font-bold text-gray-900 mb-4 flex items-center gap-2"><Flame className="w-4 h-4 text-orange-500"/> Trending</h4>
 <div className="space-y-3">
 {trendingDiscussions.slice(0,3).map(p => (
 <Link to={`/blog/${p.post._id}`} key={p.post._id} className="block text-sm font-medium text-gray-900 hover:text-[#00C08B] truncate">
 {p.post.title}
 </Link>
 ))}
 </div>
 ...
 <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 ">
 <h4 className="font-bold text-gray-900 mb-4 flex items-center gap-2"><ActivityIcon className="w-4 h-4 text-blue-500"/> Recent Activity</h4>
 <div className="space-y-3">
 {activityFeed.slice(0,3).map((act, i) => (
 <div key={i} className="flex items-center gap-2 text-sm">
 <span className="text-xs text-gray-500 w-12 shrink-0">{formatRelativeTime(act.createdAt)}</span>
 <span className="text-gray-900 truncate"><span className="font-semibold">{act.userName}</span> {act.message}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 {/* Online Members Pill */}
 <div className="flex items-center gap-3 bg-white px-5 py-4 rounded-xl border border-gray-200 shadow-sm shadow-green-500/5">
 <div className="relative flex h-3 w-3">
 <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
 <span className="relative inline-flex rounded-full h-3 w-3 bg-green-500"></span>
 </div>
 <p className="text-sm font-bold text-gray-900 ">
 {onlineUsers} <span className="text-gray-500 font-medium ml-1">Members Online</span>
 </p>
 </div>
 ...
 <div>
 <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Today</p>
 <div className="grid grid-cols-2 gap-3">
 <div>
 <p className="text-xl font-black text-[#00C08B]">{pulseData.today.newMembers}</p>
 <p className="text-[11px] font-bold text-gray-900 ">New Members</p>
 </div>
 <div>
 <p className="text-xl font-black text-[#00C08B]">{pulseData.today.newPosts}</p>
 <p className="text-[11px] font-bold text-gray-900 ">New Posts</p>
 </div>
 </div>
 </div>
 ...
 <span className="text-xs text-gray-500 ">Top Contributor</span>
 <span className="text-xs font-bold text-gray-900 ">{pulseData.thisWeek.topContributor}</span>
 </div>
 <div className="flex items-center justify-between">
 <span className="text-xs text-gray-500 ">Active Category</span>
 <span className="text-xs font-bold text-gray-900 ">{pulseData.thisWeek.mostActiveCategory}</span>
 </div>
 </div>
 </div>
 </div>
 ...
 <p className="text-xl font-black text-gray-900 ">{formatNumber(communityStats.members)}</p>
 <p className="text-[10px] text-gray-500 font-bold mt-0.5 uppercase tracking-wider">Members</p>
 </div>
 <div className="p-3 bg-gray-50 rounded-lg border border-gray-200 ">
 <p className="text-xl font-black text-gray-900 ">{formatNumber(communityStats.posts)}</p>
 <p className="text-[10px] text-gray-500 font-bold mt-0.5 uppercase tracking-wider">Posts</p>
 </div>
 <div className="p-3 bg-gray-50 rounded-lg border border-gray-200 ">
 <p className="text-xl font-black text-gray-900 ">{formatNumber(communityStats.projects)}</p>
 <p className="text-[10px] text-gray-500 font-bold mt-0.5 uppercase tracking-wider">Projects</p>
 </div>
 <div className="p-3 bg-gray-50 rounded-lg border border-gray-200 ">
 <p className="text-xl font-black text-gray-900 ">{formatNumber(communityStats.certs)}</p>
 <p className="text-[10px] text-gray-500 font-bold mt-0.5 uppercase tracking-wider">Certs</p>
 </div>
 </div>
 </div>
 ...
 <h3 className="font-bold text-gray-900 ">Filters & Navigation</h3>
 <button
 onClick={() => setMobileFiltersOpen(false)}
 ...
 exit={{ x: "100%" }}
 transition={{ type: "spring", bounce: 0, duration: 0.3 }}
 className="fixed inset-y-0 right-0 w-[320px] bg-gray-50 z-50 flex flex-col shadow-2xl xl:hidden"
 >
 <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-white ">
 <h3 className="font-bold text-gray-900 ">Stats & Activity</h3>
 <button
 onClick={() => setMobileStatsOpen(false)}
 className="p-2 text-gray-500 hover:text-gray-900 rounded-md"
 >
