...
export default function Post() {
 ...
 if (isLoading) return <div className="text-center py-20 text-gray-500 font-bold">Loading Community Post...</div>;
 ...
 return (
 <div className="bg-gray-50 min-h-screen pt-8 pb-24 text-gray-900 font-sans selection:bg-now-primary selection:text-black ">
 <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
 
 {/* BACK BUTTON */}
 <button onClick={() => navigate(-1)} className="mb-6 flex items-center text-sm font-bold text-gray-500 hover:text-gray-900 transition-colors">
 ...
 {/* MAIN ARTICLE AREA */}
 <div className="flex-1 min-w-0 bg-white border border-gray-200 rounded-2xl p-8 shadow-sm">
 ...
 <span className="text-xs text-gray-500 font-medium flex items-center gap-1 ml-auto">
 ...
 className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-colors ${turnBlack ? "border-[#00C08B] text-[#00C08B] bg-[#00C08B]/10" : "border-gray-200 text-gray-500 hover:border-[#0F172A] hover:text-gray-900 "}`}
 >
 <Heart className={`w-4 h-4 ${turnBlack ? "fill-current" : ""}`} /> {formatNumber(votes)}
 </button>
 <button onClick={handleSavePost} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-gray-200 text-gray-500 hover:border-[#0F172A] hover:text-gray-900 transition-colors">
 <Bookmark className="w-4 h-4" /> Save
 </button>
 {postData?.userId === user?._id && (
 <button onClick={handleClick} className="p-1.5 rounded-lg border border-gray-200 text-gray-500 hover:border-[#0F172A] hover:text-gray-900 transition-colors">
 {moreIcon}
 </button>
 )}
 <PostMenu
 ...
 {/* Markdown Content */}
 <div className="prose prose-slate prose-lg max-w-none prose-headings:font-bold prose-headings:text-gray-900 prose-a:text-[#00C08B]">
 ...
 key={item}
 to={`/tag/${item}`}
 className="text-sm font-medium bg-gray-50 border border-gray-200 text-gray-500 px-4 py-2 rounded-lg hover:border-[#00C08B] hover:text-[#00C08B] transition-colors"
 >
 #{item}
 </Link>
 ))}
 ...
 <button 
 onClick={() => postData?.userId !== user?._id && votePost()}
 className={`flex items-center gap-2 font-bold transition-colors ${turnBlack ? "text-[#00C08B]" : "text-gray-500 hover:text-gray-900 "}`}
 >
 <Heart className={`w-5 h-5 ${turnBlack ? "fill-current" : ""}`} /> {formatNumber(votes)}
 </button>
 <button className="flex items-center gap-2 font-bold text-gray-500 hover:text-gray-900 transition-colors">
 ...
 {/* Related Posts */}
 {id && authorData?._id && (
 <div className="mt-16 pt-10 border-t-2 border-gray-200 border-dashed">
 <h3 className="text-2xl font-bold text-gray-900 mb-8">More from {authorData?.name}</h3>
 ...
 <button className="w-full bg-gray-900 text-white font-bold py-2.5 rounded-lg hover:bg-black transition-colors mb-4">
 Follow
 </button>
 )}

 <div className="grid grid-cols-2 gap-4 border-t border-gray-200 pt-4 mt-4 text-left">
 <div>
 <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">XP</p>
 <p className="font-black text-gray-900 text-lg">{formatNumber(authorData.xp || Math.floor(Math.random() * 5000))}</p>
 </div>
 <div>
 <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Followers</p>
 <p className="font-black text-gray-900 text-lg">{formatNumber(authorData.followers?.length || 0)}</p>
 </div>
 </div>
 </div>
 )}
 ...
 <h4 className="font-bold text-xs uppercase tracking-wider text-gray-900 mb-4">Table of Contents</h4>
 <div className="space-y-3">
 {toc.map(item => (
 <div key={item.id} className="text-sm font-medium text-gray-500 hover:text-[#00C08B] cursor-pointer transition-colors line-clamp-1">
 {item.text}
 </div>
 ))}
 </div>
 ...
 key={item.id} 
 onClick={() => setMobileTocOpen(false)}
 className="text-sm font-medium text-gray-500 hover:text-[#00C08B] cursor-pointer transition-colors"
 >
 {item.text}
 </div>
 )) : <div className="text-sm text-gray-500 ">No table of contents available.</div>}
 </div>
 </motion.div>
 </>
 )}
 ...
 className="flex items-center gap-2 bg-gray-900 text-white px-5 py-3 rounded-full shadow-xl text-sm font-medium hover:bg-black transition-colors"
 >
