...
export const googleAuthDirect = asyncHandler(async (req, res, next) => {
  ...
  if (credential) {
    if (!env.GOOGLE_CLIENT_ID) {
      return next(new ServerError(500, "Server misconfiguration: GOOGLE_CLIENT_ID is missing"));
    }
    ...
    try {
      ...
      if (!payload || !payload.email) {
        return next(new ServerError(401, "Invalid Google credential payload"));
      }
      ...
      return next(new ServerError(401, "Failed to verify Google credential"));
    }
  } else {
